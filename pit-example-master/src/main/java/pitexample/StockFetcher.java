...
public class StockFetcher {  
	...
	*/
	...
	public static boolean validHigh(Stock stock){
		double low = stock.getWeek52low();
		double high = stock.getWeek52high();
		return high >= low;
		
	}
	public static boolean isOnlyUSCurrency(Stock stock){
		String currency = stock.getCurrency();
		if(currency.equals("USD"))
			return true;
		return false;
	}
	public static boolean stockGrewDuringClosing(Stock stock){
		double previousCloseValue = stock.getPreviousClose();
		double todayOpenValue = stock.getOpen();
		if(previousCloseValue < todayOpenValue)
			return true;
		return false;
	}
	
	public static boolean validDayHigh(Stock stock){
	...
	public static boolean shortRatioIsNotAboveLimit(Stock stock, double limit){
		double shortRatio = stock.getShortRatio();
		if(shortRatio > limit)
			return false;
		return true;
	}
